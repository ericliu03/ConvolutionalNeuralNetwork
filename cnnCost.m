...
for imageNum = 1:numImages
  for filterNum = 1:numFilters
      ...
      activationsPooled(:,:,filterNum,imageNum) = pooledImage(1:poolDim:end,1:poolDim:end);
  end
end
...
%%% YOUR CODE HERE %%%
delta_d = - (groundTruth - probs); % 10 * numImg, delta of top layer d
delta_p = (Wd' * delta_d);% .* activationsPooled .* (1 - activationsPooled);
delta_p = reshape(delta_p, outputDim,outputDim,numFilters,numImages);
delta_c = zeros(convDim,convDim,numFilters,numImages);

for filterNum = 1:numFilters
  for imageNum = 1:numImages
    delta_c(:,:,filterNum,imageNum) = (1/poolDim^2) *...
        kron(delta_p(:,:,filterNum,imageNum), ones(poolDim)).*...
...
for imageNum = 1:numImages
  for filterNum = 1:numFilters
      ...
      image = squeeze(images(:, :, imageNum));
      delta = rot90(squeeze(delta_c(:, :, filterNum,imageNum)), 2);
      
      Wc_grad(:,:,filterNum) = Wc_grad(:,:,filterNum) + conv2(image, delta, 'valid');
